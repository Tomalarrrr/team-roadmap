...
import {
  ...
  deserializeLudo4Tokens as deserializeTokens,
  TOTAL_TOKENS,
  TRACK_SIZE,
  PLAYER_COLORS,
  START_POSITIONS,
 ...
 * own edge (HUB.r = 13.4) so the two circles read as one instrument. */
...
// Ludo v1's palette, kept in step with Ludo4Board's token gradients.
const COLOR_HEX: Record<Ludo4Color, string> = {
  red: '#ea4330', green: '#34a853', yellow: '#fbbc05', blue: '#4285f4',
};

// Channels of the same four seats, so the standings bars can tint themselves
...
const COLOR_LABELS: Record<Ludo4Color, string> = {
...
interface Ludo4GameProps {
  onClose: () => void;
  isSearchOpen: boolean;
}

export function Ludo4Game({ onClose, isSearchOpen }: Ludo4GameProps) {
  ...
  const [myColor, setMyColor] = useState<Ludo4Color | null>(null);
  const myColorRef = useRef<Ludo4Color | null>(null);
  ...
  const [playerNames, setPlayerNames] = useState<Partial<Record<Ludo4Color, string>>>({});
  ...
  const [hostColor, setHostColor] = useState<Ludo4Color>('red');
  ...
  const botColorsRef = useRef<Set<Ludo4Color>>(new Set());

  // --- Synced game state ---
  const [tokens, setTokens] = useState<TokenPosition[]>(() => deserializeTokens(INITIAL_TOKENS));
  const tokensRef = useRef(tokens);
  tokensRef.current = tokens;
  const [currentTurn, setCurrentTurn] = useState<Ludo4Color>('red');
  ...
  // The face the die keeps showing between turns (see Ludo4GameState.lastRoll)
  const [lastRoll, setLastRoll] = useState<number | null>(null);
  const [consecutiveSixes, setConsecutiveSixes] = useState(0);
  const consecutiveSixesRef = useRef(consecutiveSixes);
  consecutiveSixesRef.current = consecutiveSixes;
  const [winner, setWinner] = useState<Ludo4Color | null>(null);
  const winnerRef = useRef(winner);
  winnerRef.current = winner;
  const [finishOrder, setFinishOrder] = useState<Ludo4Color[]>([]);
  ...
  const scheduleRipple = useCallback((coords: [number, number], color: Ludo4Color, delay: number) => {
    const t = setTimeout(() => {
  ...
  useEffect(() => {
    ...
    subscribeToGame(gameCode, (state: Ludo4GameState | null) => {
      ...
      setFinishOrder(
        state.finishOrder ? (state.finishOrder.split(',').filter(Boolean) as Ludo4Color[]) : []
      );

      const names: Partial<Record<Ludo4Color, string>> = {};
      for (const color of PLAYER_COLORS) {
        const player = state.players[color];
      ...
      console.error('[Ludo4] Subscription failed:', err);
  ...
  // --- Auto-join from URL parameter (?ludo4=CODE) ---
  useEffect(() => {
    const params = new URLSearchParams(window.location.search);
    const code = params.get('ludo4');
  ...
  const displayName = (color: Ludo4Color) => playerNames[color] ?? COLOR_LABELS[color];
  ...
  // player's arm points straight down (see Ludo4Board), which makes this fixed
  // rather than a guess: your arm is at the bottom, the others at 90° steps
  // round. Each card takes the corner clockwise-adjacent to its own arm, so
  // the panel never has to say which colour is where.
  const seatCorner = (color: Ludo4Color) => {
    ...
    >
      <div className={styles.titleBar} onMouseDown={handleDragStart} onTouchStart={handleDragStart}>
        ...
        <span className={styles.titleButtons}>
          ...
          <button className={styles.closeBtn} onClick={onClose} aria-label="Close Ludo 4">
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
      ...
      {/* How to play. Keep this in step with ludo4GameLogic. It is the only
      ...
      <div className={styles.gameArea}>
        ...
        {gamePhase === 'lobby' && (
          <div className={styles.lobby}>
            <div className={styles.lobbyTitle}>Four-player Ludo, on the round</div>
          ...
          <div className={styles.lobby}>
            ...
            {gameCode && (
              <button
                ...
                onClick={() => {
                  const url = new URL(window.location.href);
                  url.searchParams.set('ludo4', gameCode);
                ...
                return (
                  ...
                  >
                    ...
                    {/* This seat's own roll tally. Per-card rather than one
                        ...
                        score — and with a fair die (see ludo4Api), over a long

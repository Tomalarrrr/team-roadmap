...
export type { Ludo4Color, Ludo4GameState, Ludo4MoveUpdate, Unsubscribe };

const STALE_GAME_AGE_MS = 24 * 60 * 60 * 1000;
const BOT_NAMES: Record<Ludo4Color, string> = {
  red: 'Bot Red',
  green: 'Bot Green',
  yellow: 'Bot Yellow',
  blue: 'Bot Blue',
 ...
 */
function packSeats(players: Ludo4GameState['players']): Ludo4GameState['players'] {
  const seated = PLAYER_COLORS.filter((c) => !!players[c]);
  const packed: Ludo4GameState['players'] = {};
...
function seatOf(players: Ludo4GameState['players'], sessionId: string): Ludo4Color | undefined {
...
export async function createGame(
  sessionId: string,
  userName: string
): Promise<{ code: string; color: Ludo4Color }> {
  for (let attempt = 0; attempt < 5; attempt++) {
    const code = generateGameCode();
    // The first seat, not a random one — see packSeats.
    const color = PLAYER_COLORS[0];

    const initialState: Ludo4GameState = {
...
async function cleanupStaleGames(): Promise<void> {
  try {
    const games = await proxyGet<Record<string, Ludo4GameState>>('ludo4');
    if (!games) return;
    const now = Date.now();
    const removals: Promise<void>[] = [];
    for (const [code, game] of Object.entries(games)) {
      if (game.createdAt && now - game.createdAt > STALE_GAME_AGE_MS) {
        removals.push(proxyRemove(`ludo4/${code}`));
      }
    }
    if (removals.length > 0) {
      await Promise.all(removals);
      console.log(`[Ludo4] Cleaned up ${removals.length} stale game(s)`);
    }
  } catch (err) {
    console.warn('[Ludo4] Stale game cleanup failed:', err);
  }
}
...
function isHost(state: Ludo4GameState, sessionId: string): boolean {
  const host = state.players[state.host ?? 'red'];
  return !!host && host.sessionId === sessionId;
}

export async function addBot(code: string, color: Ludo4Color, sessionId: string): Promise<void> {
...
export async function removeBot(code: string, color: Ludo4Color, sessionId: string): Promise<void> {
  await proxyTransaction<Ludo4GameState>(`ludo4/${code}`, (current) => {
    if (!current) return current;
    if (current.startedAt) return undefined;
    if (!isHost(current, sessionId)) return undefined;
...
export async function leaveGame(code: string, sessionId: string): Promise<void> {
  ...
  if (roomIsEmpty) {
    try {
      await proxyRemove(`ludo4/${code}`);
    } catch (err) {
      console.warn('[Ludo4] Could not remove empty game:', err);
    }
  }
}
 ...
 * three people can easily end up with an empty seat between them. Close the
...
export async function startGame(code: string, sessionId: string): Promise<void> {
  await proxyTransaction<Ludo4GameState>(`ludo4/${code}`, (current) => {
     ...
     * one seat becomes another, which is why a full table measures even to within noise over 20,000 games.
...
export async function joinGame(
  ...
  sessionId: string,
  userName: string
): Promise<{ state: Ludo4GameState; assignedColor: Ludo4Color }> {
  ...
  await proxyTransaction<Ludo4GameState>(`ludo4/${code}`, (current) => {
    ...
    const openColors = current.startedAt
      ...
      : PLAYER_COLORS;
    let foundColor: Ludo4Color | null = null;
  ...
  const finalState = await proxyGet<Ludo4GameState>(`ludo4/${code}`);
  if (!finalState) throw new Error('Game not found');

  let confirmedColor: Ludo4Color | null = null;
  for (const color of PLAYER_COLORS) {
...
export function subscribeToGame(
  code: string,
  callback: (state: Ludo4GameState | null) => void,
  onConnectionChange?: (connected: boolean) => void
): Promise<Unsubscribe> {
  return subscribeToPath<Ludo4GameState>(`ludo4/${code}`, 'Ludo4', callback, onConnectionChange);
}

export async function makeMove(
  code: string,
  expectedTurn: Ludo4Color,
  updates: Ludo4MoveUpdate
): Promise<boolean> {
  const result = await proxyTransaction<Ludo4GameState>(`ludo4/${code}`, (current) => {
    if (!current) return current;
    ...
    return {
      ...
      // aren't lost (see mergeRollStats in ludo4Board). Runs on every attempt.
